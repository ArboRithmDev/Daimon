...
def build_updater_script(installer: Path, install_dir: Path, self_path: Path,
    ...
    # This is a Windows PowerShell script: the exe path must use Windows
    # separators regardless of the host that generates it (PureWindowsPath keeps
    # backslashes even when the manifest/build runs on macOS).
    return _TEMPLATE.format(
        delay=delay_ms,
        installer=str(installer),
        exe=str(PureWindowsPath(install_dir) / "Daimon.exe"),
        self=str(self_path),
    )
