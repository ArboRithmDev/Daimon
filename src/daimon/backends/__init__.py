...
def build_focus_probe():
    """Live foreground-window probe for the four-state focus result."""
    if sys.platform == "darwin":
        from ..motor.focus import MacOSFocusProbe
        return MacOSFocusProbe()
    if sys.platform == "win32":
        from ..motor.focus import WindowsFocusProbe
